...
ax.set_title("Bahn des Flansches")
ax.legend()

ax = fig.add_subplot(1, 3, 2)
ax.plot(t / T, abweichung(Pj), lw=2, label="MoveJ")
ax.plot(t / T, abweichung(Pl), lw=2, label="MoveL")
ax.set_xlabel("τ"); ax.set_ylabel("Abweichung von der Geraden [mm]")
ax.set_title("Bahnabweichung"); ax.grid(alpha=.3); ax.legend()

ax = fig.add_subplot(1, 3, 3)
for i in range(6):
    linie = ax.plot(t, qd_j[:, i], lw=1.5, label=f"Gelenk {i+1}")
    ax.plot(t, qd_l[:, i], lw=1.5, ls="--", color=linie[0].get_color())
ax.set_xlabel("t [s]"); ax.set_ylabel("q̇ [°/s]")
ax.set_title("Gelenkgeschwindigkeiten (durchgezogen MoveJ, gestrichelt MoveL)")
